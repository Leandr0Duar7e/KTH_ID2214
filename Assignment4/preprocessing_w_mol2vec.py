...
from rdkit.Chem import Descriptors, Fragments, Lipinski, rdFingerprintGenerator
...
def expand_electronic_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    new_df["n_aliphatic_rings"] = mols.swifter.apply(
        rdMolDescriptors.CalcNumAliphaticRings
    )
    new_df["n_saturated_rings"] = mols.swifter.apply(
...
def expand_mol2vec_features(
    df: pd.DataFrame, model_path: str = "models/model_300dim.pkl"
) -> pd.DataFrame:
    """
...
class StatisticalPreprocessing:
    ...
    def fit_transform_all_modes(
        ...
        """
        logging.info(
            f"Starting fit_transform_all_modes with max_features={max_features}"
        )
        ...
        for mode in ["nbest", "FPR", "FDR", "FWER", "model_based"]:
            try:
                ...
                if mode == "nbest":
                    ...
                    selected_features = X_sample.columns[
                    ...
                    if mode == "FPR":
                        ...
                        reject, _, _, _ = multipletests(
                            p_values, method="fdr_bh", alpha=0.8
                        )
                        threshold = reject
                    else:  # FWER
                        reject, _, _, _ = multipletests(
                            p_values, method="bonferroni", alpha=0.6
                        )
                        threshold = reject
                    ...
                    if len(selected_features) < 10:
                        logging.info(
                            f"Too few features selected ({len(selected_features)}), falling back to top {max_features}"
                        )
                        selected_features = scores.nlargest(max_features).index.tolist()
                ...
                scores = cross_val_score(
                    clf, X_selected, y_sample, cv=cv, scoring="roc_auc"
                )
    ...
    try:
        ...
        if transformed_df is None:
            ...
            feature_sets = stat_preprocessor.fit_transform_all_modes(
                train_df, max_features=300, expand=True
            )
        else:
        ...
        best_columns = [
            col
            for col in stat_preprocessor.columns[stat_preprocessor.best_mode]
            if col != "INDEX"
        ]
        ...
        best_columns = [
            col for col in stat_preprocessor.columns[best_mode] if col != "INDEX"
        ]
